...
APP_NAME="overseerr"
IMAGE_NAME="overseerr-mac:latest"
CONTAINER_NAME="overseerr-MAC"
APP_PORT=5055
USE_VPN=false

log_info() {
    echo -e "\033[0;34m[OVERSEERR]\033[0m $1"
}
...
# Build image
build_app() {
    log_info "Building Overseerr Docker image..."
    ...
    log_success "Overseerr image built successfully"
}

# Deploy container
deploy_app() {
    log_info "Deploying Overseerr container..."
    ...
    docker run -d \
        ...
        --label="net.unraid.docker.icon=https://raw.githubusercontent.com/linuxserver/docker-templates/master/linuxserver.io/img/overseerr-logo.png" \
    ...
    log_success "Overseerr container deployed"
}

# Store in Vault
store_in_vault() {
    log_info "Storing Overseerr information in Vault..."
...
# Main
main() {
    log_info "Starting Overseerr deployment..."
    
    build_app
    deploy_app
    store_in_vault
    
    log_success "Overseerr deployment complete!"
