...
APP_NAME="unmanic"
IMAGE_NAME="unmanic-mac:latest"
CONTAINER_NAME="unmanic-MAC"
APP_PORT=8888
USE_VPN=false

log_info() {
    echo -e "\033[0;34m[UNMANIC]\033[0m $1"
}
...
# Build image
build_app() {
    log_info "Building Unmanic Docker image..."
    ...
    log_success "Unmanic image built successfully"
}

# Deploy container
deploy_app() {
    log_info "Deploying Unmanic container..."
    ...
    docker run -d \
        ...
        --label="net.unraid.docker.icon=https://dashboardicons.com/assets/img/icons/unmanic.png" \
    ...
    log_success "Unmanic container deployed"
}

# Store in Vault
store_in_vault() {
    log_info "Storing Unmanic information in Vault..."
...
# Main
main() {
    log_info "Starting Unmanic deployment..."
    
    build_app
    deploy_app
    store_in_vault
    
    log_success "Unmanic deployment complete!"
