...
APP_NAME="prowlarr"
IMAGE_NAME="prowlarr-mac:latest"
CONTAINER_NAME="prowlarr-MAC"
APP_PORT=9117
USE_VPN=true

log_info() {
    echo -e "\033[0;34m[PROWLARR]\033[0m $1"
}
...
# Build image
build_app() {
    log_info "Building Prowlarr Docker image..."
    ...
    log_success "Prowlarr image built successfully"
}

# Deploy container
deploy_app() {
    log_info "Deploying Prowlarr container..."
    ...
    docker run -d \
        ...
        --label="net.unraid.docker.icon=https://raw.githubusercontent.com/linuxserver/docker-templates/master/linuxserver.io/img/prowlarr-logo.png" \
    ...
    log_success "Prowlarr container deployed"
}

# Store in Vault
store_in_vault() {
    log_info "Storing Prowlarr information in Vault..."
...
# Main
main() {
    log_info "Starting Prowlarr deployment..."
    
    build_app
    deploy_app
    store_in_vault
    
    log_success "Prowlarr deployment complete!"
