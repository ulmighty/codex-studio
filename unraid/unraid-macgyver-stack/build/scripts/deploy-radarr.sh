...
APP_NAME="radarr"
IMAGE_NAME="radarr-mac:latest"
CONTAINER_NAME="radarr-MAC"
APP_PORT=7878
USE_VPN=true

log_info() {
    echo -e "\033[0;34m[RADARR]\033[0m $1"
}
...
# Build image
build_app() {
    log_info "Building Radarr Docker image..."
    ...
    log_success "Radarr image built successfully"
}

# Deploy container
deploy_app() {
    log_info "Deploying Radarr container..."
    ...
    docker run -d \
        ...
        --label="net.unraid.docker.icon=https://raw.githubusercontent.com/linuxserver/docker-templates/master/linuxserver.io/img/radarr-logo.png" \
    ...
    log_success "Radarr container deployed"
}

# Store in Vault
store_in_vault() {
    log_info "Storing Radarr information in Vault..."
...
# Main
main() {
    log_info "Starting Radarr deployment..."
    
    build_app
    deploy_app
    store_in_vault
    
    log_success "Radarr deployment complete!"
