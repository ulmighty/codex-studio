...
APP_NAME="bazarr"
IMAGE_NAME="bazarr-mac:latest"
CONTAINER_NAME="bazarr-MAC"
APP_PORT=6767
USE_VPN=true

log_info() {
    echo -e "\033[0;34m[BAZARR]\033[0m $1"
}
...
# Build image
build_app() {
    log_info "Building Bazarr Docker image..."
    ...
    log_success "Bazarr image built successfully"
}

# Deploy container
deploy_app() {
    log_info "Deploying Bazarr container..."
    ...
    docker run -d \
        ...
        --label="net.unraid.docker.icon=https://raw.githubusercontent.com/linuxserver/docker-templates/master/linuxserver.io/img/bazarr-logo.png" \
    ...
    log_success "Bazarr container deployed"
}

# Store in Vault
store_in_vault() {
    log_info "Storing Bazarr information in Vault..."
...
# Main
main() {
    log_info "Starting Bazarr deployment..."
    
    build_app
    deploy_app
    store_in_vault
    
    log_success "Bazarr deployment complete!"
