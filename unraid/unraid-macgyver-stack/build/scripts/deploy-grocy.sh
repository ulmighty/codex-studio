...
APP_NAME="grocy"
IMAGE_NAME="grocy-mac:latest"
CONTAINER_NAME="grocy-MAC"
APP_PORT=9283
USE_VPN=false

log_info() {
    echo -e "\033[0;34m[GROCY]\033[0m $1"
}
...
# Build image
build_app() {
    log_info "Building Grocy Docker image..."
    ...
    log_success "Grocy image built successfully"
}

# Deploy container
deploy_app() {
    log_info "Deploying Grocy container..."
    ...
    docker run -d \
        ...
        --label="net.unraid.docker.icon=https://raw.githubusercontent.com/linuxserver/docker-templates/master/linuxserver.io/img/grocy-logo.png" \
    ...
    log_success "Grocy container deployed"
}

# Store in Vault
store_in_vault() {
    log_info "Storing Grocy information in Vault..."
...
# Main
main() {
    log_info "Starting Grocy deployment..."
    
    build_app
    deploy_app
    store_in_vault
    
    log_success "Grocy deployment complete!"
