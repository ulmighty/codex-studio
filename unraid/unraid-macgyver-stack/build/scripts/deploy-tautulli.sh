...
APP_NAME="tautulli"
IMAGE_NAME="tautulli-mac:latest"
CONTAINER_NAME="tautulli-MAC"
APP_PORT=8181
USE_VPN=false

log_info() {
    echo -e "\033[0;34m[TAUTULLI]\033[0m $1"
}
...
# Build image
build_app() {
    log_info "Building Tautulli Docker image..."
    ...
    log_success "Tautulli image built successfully"
}

# Deploy container
deploy_app() {
    log_info "Deploying Tautulli container..."
    ...
    docker run -d \
        ...
        --label="net.unraid.docker.icon=https://raw.githubusercontent.com/linuxserver/docker-templates/master/linuxserver.io/img/tautulli-logo.png" \
    ...
    log_success "Tautulli container deployed"
}

# Store in Vault
store_in_vault() {
    log_info "Storing Tautulli information in Vault..."
...
# Main
main() {
    log_info "Starting Tautulli deployment..."
    
    build_app
    deploy_app
    store_in_vault
    
    log_success "Tautulli deployment complete!"
