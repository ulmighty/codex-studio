...
APP_NAME="readarr"
IMAGE_NAME="readarr-mac:latest"
CONTAINER_NAME="readarr-MAC"
APP_PORT=8787
USE_VPN=true

log_info() {
    echo -e "\033[0;34m[READARR]\033[0m $1"
}
...
# Build image
build_app() {
    log_info "Building Readarr Docker image..."
    ...
    log_success "Readarr image built successfully"
}

# Deploy container
deploy_app() {
    log_info "Deploying Readarr container..."
    ...
    docker run -d \
        ...
        --label="net.unraid.docker.icon=https://raw.githubusercontent.com/linuxserver/docker-templates/master/linuxserver.io/img/readarr-logo.png" \
    ...
    log_success "Readarr container deployed"
}

# Store in Vault
store_in_vault() {
    log_info "Storing Readarr information in Vault..."
...
# Main
main() {
    log_info "Starting Readarr deployment..."
    
    build_app
    deploy_app
    store_in_vault
    
    log_success "Readarr deployment complete!"
