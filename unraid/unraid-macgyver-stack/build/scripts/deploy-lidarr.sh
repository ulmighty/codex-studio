...
APP_NAME="lidarr"
IMAGE_NAME="lidarr-mac:latest"
CONTAINER_NAME="lidarr-MAC"
APP_PORT=8686
USE_VPN=true

log_info() {
    echo -e "\033[0;34m[LIDARR]\033[0m $1"
}
...
# Build image
build_app() {
    log_info "Building Lidarr Docker image..."
    ...
    log_success "Lidarr image built successfully"
}

# Deploy container
deploy_app() {
    log_info "Deploying Lidarr container..."
    ...
    docker run -d \
        ...
        --label="net.unraid.docker.icon=https://raw.githubusercontent.com/linuxserver/docker-templates/master/linuxserver.io/img/lidarr-logo.png" \
    ...
    log_success "Lidarr container deployed"
}

# Store in Vault
store_in_vault() {
    log_info "Storing Lidarr information in Vault..."
...
# Main
main() {
    log_info "Starting Lidarr deployment..."
    
    build_app
    deploy_app
    store_in_vault
    
    log_success "Lidarr deployment complete!"
