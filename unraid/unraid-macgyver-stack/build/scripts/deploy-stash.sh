...
APP_NAME="stash"
IMAGE_NAME="stash-mac:latest"
CONTAINER_NAME="stash-MAC"
APP_PORT=9999
USE_VPN=false

log_info() {
    echo -e "\033[0;34m[STASH]\033[0m $1"
}
...
# Build image
build_app() {
    log_info "Building Stash Docker image..."
    ...
    log_success "Stash image built successfully"
}

# Deploy container
deploy_app() {
    log_info "Deploying Stash container..."
    ...
    docker run -d \
        ...
        --label="net.unraid.docker.icon=https://raw.githubusercontent.com/stashapp/stash/master/ui/v2.5/src/assets/images/stash-logo.png" \
    ...
    log_success "Stash container deployed"
}

# Store in Vault
store_in_vault() {
    log_info "Storing Stash information in Vault..."
...
# Main
main() {
    log_info "Starting Stash deployment..."
    
    build_app
    deploy_app
    store_in_vault
    
    log_success "Stash deployment complete!"
