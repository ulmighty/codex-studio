...
- `projects/FacialRecog/packages/audio/` – EVP capture, preprocessing, Whisper wrappers, and embedding utilities.
- `projects/FacialRecog/configs/pipeline.yaml` – read-only for model paths; coordinate changes with BACKEND_API before editing.
...
## Acceptance Checks
- Offline transcription demo (`python -m projects.FacialRecog.packages.audio.cli sample.wav`) finishes without network access, storing disclaimer-tagged transcripts.
...
- `ruff check projects/FacialRecog/packages/audio projects/FacialRecog/tests/audio`
