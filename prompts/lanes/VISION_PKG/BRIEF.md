...
- `projects/FacialRecog/packages/vision/` – core inference code (detection, embedding, post-processing utilities).
- `projects/FacialRecog/configs/pipeline.yaml` – read-only for model path references; treat updates as coordinated changes with BACKEND_API.
...
## Acceptance Checks
- CLI smoke test (`python -m projects.FacialRecog.packages.vision.cli sample.mp4`) completes without network access and emits blurred preview frames.
...
- `ruff check projects/FacialRecog/packages/vision projects/FacialRecog/tests/vision`
